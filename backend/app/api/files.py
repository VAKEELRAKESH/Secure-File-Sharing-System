...
from app.schemas.file import FileResponse, FolderCreate, FolderResponse, FileVersionResponse
...
async def upload_file(
    ...
    # Server-Side AES-256 Encryption Workflow
...
def download_file(
    ...
    # Temporarily decrypt ciphertext in memory for delivery
    try:
...
def create_folder(
    ...
    folder = FolderModel(name=folder_in.name, parent_id=folder_in.parent_id, owner_id=current_user.id)
    db.add(folder)
    db.commit()
    db.refresh(folder)
    return folder
